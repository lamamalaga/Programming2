...
namespace PhotoEnhancer
{
    static class Program
    {
        ...
        static void Main()
        {
            ...
            mainForm.AddFilter(new PixelFilter<SepiaParameters>("Сепия", (pixel, parameters) =>
            {
                var OriginalHue = Convertors.GetPixelHue(pixel);
                var OriginalSaturation = Convertors.GetPixelSaturation(pixel);
                var OriginalLightness = Convertors.GetPixelLightness(pixel);
            ...
            mainForm.AddFilter(new TransformFilter("Замена четных строк",
                size => size,
                (point, size) =>
                {
                    if (point.Y % 2 == 0)
            ...
            mainForm.AddFilter(new TransformFilter<ReductionInSizeParameters>("Уменьшение изображения", new ReductionInSizeTransformers()));
